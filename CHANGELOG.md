...
전체 코드 감사(6개 차원 · 적대적 검증)와 프리모템을 돌려 나온 결과를 반영한 개편 릴리스.
**새 기능보다 "우리가 하고 있는 말을 사실로 만드는 것"에 초점**을 뒀다.

> ⚠️ **동작이 바뀐 부분** — 나이 필터가 켜져 있으면 휴지통 모드를 쓸 수 없습니다(아래 사유 참조).
> 프로젝트 스캐너에서 git 추적 중인 폴더는 기본 선택에서 빠집니다.

### 🩹 화면 약속과 실제 삭제가 어긋나던 문제 (중요)
- **나이 필터 + 휴지통**: `30일+ 만 지움` 배지를 띄우고 실제로는 캐시 디렉터리를 **통째로** 휴지통으로 옮기고 있었다. 완료창의 '방금 항목만 영구삭제'를 누르면 최근 파일까지 사라졌다. 휴지통은 폴더 단위로 옮기는 방식이라 나이 필터를 지킬 수 없으므로, **나이 필터가 켜져 있으면 휴지통 모드를 비활성화**하고 이유를 표시한다. UI 뿐 아니라 **삭제를 수행하는 엔진에서도 강제**해 저장된 설정값으로 우회되지 않게 했다
- **'복구 가능'이 거짓이던 경로**: `docker`·`xcode-sim`·`simruntime`·`rustup-targets` 는 경로가 없어 휴지통 모드에서도 네이티브 명령으로 영구 삭제된다. 해당 항목 선택 시 "휴지통을 지원하지 않아 영구 삭제됩니다"를 명시
- **나이 필터 정리의 실패 보고**: 권한 문제로 하나도 못 지워도 '완료'로 보고되고 회수량이 가산되던 문제 수정(실패 시 비0 반환)
- **swiftpm**: 캐시가 아닌 사용자 설정 디렉터리(`~/Library/org.swift.swiftpm` — 레지스트리·미러·서명 신뢰)를 정리 대상에서 제외
- **프로젝트 스캐너**: git 이 추적 중인 내용이 든 폴더(소스를 커밋해 두는 `dist/`·`build/`)에 경고 배지 + **기본 선택 해제**

### 🔒 프라이버시 서약을 코드가 지키는 수준으로 정정
- 기존 문구는 14/15개 언어에서 무조건부("어떠한 경우에도 내용을 읽지 않는다")였는데, git 히스토리 검사는 gitleaks 가 내용을 읽고 시크릿 평문이 담긴 리포트를 DevSweep 이 만들어 파싱하고 있었다. **약속이 방어 수준보다 크게 말하고 있던 것을 정확하게 다시 썼다**(앱 정보탭 15개 언어 + README 5종)
- **gitleaks 파이프라인 교체 — 이제 시크릿 값을 아예 전달받지 않는다**: JSON 리포트를 임시파일로 받아 걸러내는 대신, `--report-template` 으로 **필요한 메타데이터만 출력하게** 했다. 시크릿 필드가 애초에 생성되지 않으므로 평문이 파이프에도 디스크에도 닿지 않는다(비정상 종료 시 평문 잔존 위험도 소멸)
- 같은 변경으로 **`python3` 의존 제거** — 미설치 환경에서 검사가 조용히 '0건'이 되던 거짓 안심 경로가 사라졌다

### 🛡 "발견 없음"과 "안전함"을 구분
- 빈 스캔 결과를 `위험한 민감 파일 없음` 이라고 단언하던 것을 **`스캔 범위에서 발견 없음` + 제외된 범위 명시**로 변경. 화이트리스트 스캐너의 빈 결과는 부재 증명이 아니다
- git 히스토리 검사에서 **일부 저장소 검사 실패를 '0건'과 분리 집계**하고 경고로 표시("N개 검사 못 함 — '이상 없음'이 아닙니다")
- 민감 파일 패턴 확장: `.envrc`(direnv) · `prod.env` 류 접미형 · `*.tfvars` · `.terraformrc` · `firebase-adminsdk*.json`

### 🔔 실시간 감시 — 첫 실행 동의 + 폭주 방지
- 기본 켜짐은 유지하되 **최초 1회 무엇을 하는 기능인지 알리고 시작**한다. 안내를 보기 전에는 감시가 시작되지 않는다
- 대량 파일 생성(clone·checkout·백업 복원) 시 배치 상한(300)과 알림 상한(3건 + 요약)을 두고, 초과 시 **"전체 스캔 권장"** 으로 안내 — 침묵하는 누락을 만들지 않기 위해
- 다운로드 랜딩에 보안 점검·실시간 감시 항목 추가(이전에는 언급이 전혀 없었다)

### 📄 저장소 정비
- **LICENSE 추가** — README 5종이 MIT 를 표방했으나 파일이 없어 법적으로는 all rights reserved 상태였다
- **SECURITY.md 추가** — 비공개 취약점 신고 창구, 대응 기대치, 그리고 **알려진 한계**(미공증 배포·화이트리스트 방식·기본 제외 범위)를 명시
- **`build.sh` 저장소 복귀** — 배포 바이너리와 공개 소스의 일치를 제3자가 검증할 수 있도록

### 🔴 민감 파일 실시간 감시 (FSEvents)
- 새 `.env`·키 파일이 **위험한 상태로 생기는 순간** 알림 — gitignore 안 됨(높음) / 이미 커밋됨(심각) / 권한 느슨(보통). **커밋되기 전에 잡는 것**이 이 기능의 목적
- **3단 필터로 비용 제거**: ① Swift 패턴 매칭(프룬 경로·확장자·경로 게이팅) → ② 디바운스 0.8초(에디터 저장 시 다발 이벤트 병합) → ③ 살아남은 경로만 CLI `check-secret` 정밀 판정. 홈 전체 감시에도 CPU 부담 없음
- **노이즈 억제 3중**: 스캔 시점에 이미 있던 파일 제외(본인 편집 알림 차단) · 세션 내 경로별 1회 · `low` 등급 무시. `.env.example` 류는 필터 단계에서 탈락
- 감지 항목은 보안 탭 목록에 자동 합류. 설정 > 일반에 토글(**기본 켜짐** — 위험을 미연에 막는 것이 목적이므로 끄는 쪽을 사용자 선택으로), 보안 탭 툴바에 `● 감시 중` 표시
- **프라이버시 불변**: FSEvents 는 경로만 통보하며 파일 내용은 읽지 않음 — 기존 서약 그대로 유효
- CLI `check-secret <path>` 신규 (단일 파일 판정, 기존 `secret_kind`·`git_leak_state` 재사용)
- 검증: 경로 필터 단위 테스트 24/24 통과(프룬·게이팅·제외 규칙), CLI 5케이스 통과

## 2026-08-03 (v0.2.1)

### 🕐 git 히스토리 시크릿 검사 (gitleaks 위임)
- 보안 탭 하단 **[git 히스토리]** — 스캔 루트 아래 git repo 를 찾아 커밋 히스토리에 묻힌 시크릿을 검사. 안내 → 진행 → 결과 3상태 모달, gitleaks 미설치 시 `brew install gitleaks` 안내(복사 버튼)
- 🔴 **시크릿 원문 미보유(이중 방어)**: gitleaks 리포트에는 시크릿 평문(`Secret`/`Match`)이 들어있으므로, CLI `cmd_scan_git_secrets` 가 필요한 키만 화이트리스트 재조립해 방출(원문이 파이프에 실리지 않음) + Swift `GitLeakFinding` 에 해당 프로퍼티를 두지 않아 디코딩 단계에서 재차 폐기. 임시 리포트는 `trap ... RETURN` 으로 어떤 종료 경로에서도 삭제
- 결과는 리포트 전용 — 화면에는 `규칙 · 파일 · 커밋 · 작성자 · 날짜`만, 값은 표시하지 않음. "히스토리 시크릿은 파일 삭제로 사라지지 않음 → 키 폐기·재발급" 경고 동반
- 외부 도구는 선택 사항 — 미설치여도 앱 나머지 기능은 정상 동작(`available:false` 안내만)
- 검증: 픽스처 2 repo(github-pat·slack-bot-token) 탐지, **시크릿 원문 누출 0건**, 미설치·없는 경로·repo 없는 폴더 엣지 케이스, 임시 리포트 잔존 0

### 🔒 프라이버시 서약 명문화 · 정리
- 설정 > 정보에 프라이버시 서약 섹션 추가(민감정보 미열람·무전송, 15언어). README 5종에도 동일 문구 + 업데이트 네트워크 사용 범위 명시
- 가상 번역 크레딧 섹션 제거 · `build.sh`/`TODO.md` 를 저장소에서 제외(로컬 전용) · README 에 로드맵(완료/지원 예정) 추가
