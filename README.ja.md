...
> 🛡 **プライバシー** — DevSweep はファイルの内容を読みません。名前・場所・権限・git の状態のみを確認します。git 履歴の検査は実行したときだけ外部ツール [gitleaks](https://github.com/gitleaks/gitleaks) が行い、DevSweep はメタデータのみを要求するため**シークレットの値を受け取りません**。ネットワークは GitHub のアップデート確認・ダウンロードにのみ使用し（設定でオフにできます）、スキャン結果やファイル情報を送信することはありません。
