...
> 🛡 **隱私** — DevSweep 不讀取檔案內容，只查看名稱、位置、權限與 git 狀態。git 歷史掃描僅在你啟動時由外部工具 [gitleaks](https://github.com/gitleaks/gitleaks) 執行，DevSweep 只索取中繼資料，因此**不會取得密鑰內容**。網路僅用於從 GitHub 檢查與下載更新（可在設定中關閉），掃描結果與檔案資訊不會傳送到任何地方。
...
- **即時監視** — 當新的 `.env` 或密鑰檔案以危險狀態（未加入 gitignore、權限寬鬆）出現時，**在被提交之前**提醒你。預設開啟，可在設定中關閉。透過 FSEvents 只監視路徑，絕不讀取檔案內容
