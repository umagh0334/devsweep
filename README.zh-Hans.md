...
> 🛡 **隐私** — DevSweep 不读取文件内容，只查看名称、位置、权限和 git 状态。git 历史扫描仅在你启动时由外部工具 [gitleaks](https://github.com/gitleaks/gitleaks) 执行，DevSweep 只索取元数据，因此**不会拿到密钥内容**。网络仅用于从 GitHub 检查和下载更新（可在设置中关闭），扫描结果与文件信息不会发送到任何地方。
...
- **git 历史扫描** — 通过委托 [gitleaks](https://github.com/gitleaks/gitleaks) 查找埋在提交历史中的密钥（可选，未安装也不影响使用）。只获取类型、文件、提交和日期，**密钥内容本身既不保存也不显示**
- **实时监视** — 当新的 `.env` 或密钥文件以危险状态（未加入 gitignore、权限宽松）出现时，**在被提交之前**提醒你。默认开启，可在设置中关闭。通过 FSEvents 只监视路径，绝不读取文件内容
...
devsweep scan-git-secrets ~   # 用 gitleaks 扫描 git 历史——仅元数据，不输出密钥内容
devsweep check-secret <path>  # 判定单个文件（实时监视使用）
```
...
构建脚本不包含在仓库中——请从 [Releases](https://github.com/umagh0334/devsweep/releases) 获取成品应用。

自行构建也不难：用 `swiftc`（Xcode 命令行工具，无需完整 Xcode）编译 `Sources/*.swift`，将 `Resources/Info.plist`、`devsweep` 引擎和图标打包为 `.app`，再进行 ad-hoc 签名即可。macOS 14+，**universal binary**（Apple Silicon + Intel）。
...
**已完成**
- ✅ 保护列表 · 时间过滤（`--older-than`）· 定时自动清理（launchd）
- ✅ 44 个缓存类别 · 项目文件夹扫描器 · 安全检查（单个/批量修复）
- ✅ git 历史密钥扫描（集成 gitleaks）
- ✅ 敏感文件实时监视（FSEvents）
- ✅ 废纸篓模式 · 菜单栏与后台模式 · 个性化主页仪表盘
- ✅ Ed25519 签名自动更新 · 15 种语言

**计划中**
- 清理历史——"何时 / 清了什么 / 释放多少"
- 用户自定义类别
- 更精确的回收空间测量
- Developer ID 签名与公证（告别 Gatekeeper 麻烦）
